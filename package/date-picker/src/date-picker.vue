      ...
      <div class="calendar">
        <div class="calendar-head">
          ...
          <a
            ...
            @click="_monthClick(0)"
            v-if="showMonthControl && activePanel != 'Year'"
            >‹</a
          >
          <a class="calendar-year-select" @click="activePanel = 'Year'"
            >{{ year }}年</a
          >
          <a
            class="calendar-year-month"
            @click="activePanel = 'Month'"
            v-if="showMonth"
            >{{ month }}月</a
          >
          <a
...
export default {
  ...
  methods: {
    _yearClick(type) {
      ...
      if (this.activePanel === "Year") {
        this.bodyValue = new Date(
          this.bodyValue.setYear(this.bodyValue.getFullYear() + num)
        );
      } else {
        this.activeValue = new Date(
          this.activeValue.setYear(this.activeValue.getFullYear() + num)
        );
      }
    },
    _monthClick(type) {
      ...
      this.activeValue = new Date(
        this.activeValue.setMonth(this.activeValue.getMonth() + num)
      );
    },
    _getTime(date) {
      // 判断初始日期转为标准格式
